...
def infeasible_key(individual: Individual) -> Tuple[int, int, int]:
    ...
    unmet_required = sum(
        0 if value else 1 for key, value in penalty_flags.items() if key != "is_feasible"
    )
    return (
...
def _metric_extrema(population: Sequence[Individual]) -> Tuple[Optional[float], Optional[float], Optional[float]]:
    ...
    return (
        ...
        max(evaluation.emptiness for evaluation in feasible_evaluations),
    )
...
def run_minimal_ea(cfg: MarioConfig) -> Tuple[List[Individual], List[GenerationLog]]:
    ...
    for generation in range(cfg.generations):
        ...
        best_difficulty_error, best_structural_diversity, best_emptiness = _metric_extrema(population)
        logs.append(
            GenerationLog(
