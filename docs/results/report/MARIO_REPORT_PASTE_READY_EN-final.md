...
### E. Parameter Scan
A broader parameter scan over population sizes 20, 30, and 40 and mutation rates 0.1, 0.2, and 0.3 further confirms that the system has moved beyond a one-off demo. Across these settings, average difficulty error ranges from 0.3833 to 0.4583, average emptiness error ranges from 0.3089 to 0.3365, average first-front hypervolume ranges from 0.2473 to 0.2667, and average front spread ranges from 0.0203 to 0.0527. No single configuration dominates all criteria. Population 40 with mutation 0.1 gives the best average difficulty error, population 40 with mutation 0.3 gives the best average emptiness error, and population 20 with mutation 0.3 gives the highest hypervolume and widest front spread. This again reinforces the core message of the project: different parameter settings emphasize different trade-offs, so tuning should be understood as shaping frontier behavior rather than searching for one universally best setting.
...
The experiments show that the hard-constrained search pipeline is stable in the baseline and repaired-seeded settings, that NSGA-II is more appropriate than a single-objective EA for this setting, and that semantic objectives materially change the resulting frontier. They also show that AI-seeded initialization is genuinely integrated into the pipeline, although its performance still depends on upstream data quality and repair. Finally, the frontier browser and replay layers make the final solutions easier to inspect, compare, and present. Taken together, these components establish a solid foundation for future work on richer segment libraries, better upstream data modeling, and stronger playable evaluation.

## Project Availability
The interactive demo is available at [https://ox00.github.io/EA-Lab/](https://ox00.github.io/EA-Lab/). The source code, documents, and experiment outputs are available at [https://github.com/ox00/EA-Lab](https://github.com/ox00/EA-Lab).